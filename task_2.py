...
parser.add_argument('--data', type=str, default='data_new/target_gene', help='Dataset name')
...
def build_data(g, src, dst, edge_type, k):
    ...
    # dst_not_og = list(set(map(tuple, dst)) - set(map(tuple, dst_og.tolist())))
    new_g = g.clone()
...
g_pkg0 = ds_pkg[0]
g_kg0 = ds_kg[0]
pkg = create_doubledirection_graph(g_pkg0)
merge = create_doubledirection_graph(g_kg0)
...
files = os.listdir(args.data + '/pkg')
mrr_score = {}
...
for file in tqdm(files):
    ...
    for i in range(args.times):
        ...
        # src = [df['src'][0].tolist()]
        # dst = df['dst'].tolist()
        df_pkg = pd.read_csv(f'{args.data}/pkg/{file}')
        df_merge = pd.read_csv(f'{args.data}/kg/{file}')
        src_pkg = [df_pkg['src'][0].tolist()]
        dst_pkg = df_pkg['dst'].tolist()
        src_merge = [df_merge['src'][0].tolist()]
        dst_merge = df_merge['dst'].tolist()

        _, train_pkg, test_pkg, test_neg_pkg = build_data(pkg, src_pkg, dst_pkg, edge_type, k_test)
        _, train_merge, test_merge, test_neg_merge = build_data(merge, src_merge, dst_merge, edge_type, k_test)
...
with open(f'task_2_score/case_{run_id}_{args.times}.pkl', 'wb') as f:
